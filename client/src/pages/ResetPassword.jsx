...
const ResetPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    if (passwordError) return toast.error("Виправте помилки перед відправкою");
    if (password.length < 8) return toast.error("Мінімум 8 символів");
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] font-['Plus_Jakarta_Sans',_sans-serif] text-[var(--text-dark)]">
      ...
      <style>{`
        .bento-auth-card { background: var(--bg-card); border: 1px solid var(--border-color); position: relative; overflow: hidden; }
        .bento-auth-card::before { content: ''; position: absolute; top: 0; left: 0; width: 4px; height: 100%; background: #7c3aed; }
        .input-minimal { background: var(--bg-main); border: 1px solid var(--border-color); padding: 12px 16px 12px 48px; width: 100%; font-size: 14px; transition: all 0.2s ease; color: var(--text-dark); outline: none; }
        .input-minimal:focus { border-color: #7c3aed; background: var(--bg-card); }
        .label-mono { font-family: 'Space Mono', monospace; font-size: 10px; text-transform: uppercase; letter-spacing: 0.2em; color: var(--text-gray); font-weight: 700; }
        .grid-bg { background-image: radial-gradient(var(--border-color) 1px, transparent 1px); background-size: 32px 32px; position: absolute; inset: 0; opacity: 0.4; z-index: 0; }
      `}</style>
            ...
            {isSuccess ? (
              <div className="text-center py-6" data-aos="zoom-in">
                ...
                <p className="text-[var(--text-gray)] text-[11px] font-medium uppercase tracking-wider mb-8 leading-relaxed">
                  Ваш доступ успішно відновлено. Перенаправлення...
                </p>
                <Link
                  to="/login"
                ...
                <form onSubmit={handleReset} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      />
                      <input
                        className={`input-minimal ${passwordError ? "border-red-500" : ""}`}
                        type={showPassword ? "text" : "password"}
                        placeholder="••••••••"
                  ...
                  <button
                    ...
                    className="w-full mt-4 py-4 bg-purple-600 text-white font-bold text-xs uppercase tracking-[0.2em] hover:bg-purple-700 disabled:opacity-50 transition-all flex items-center justify-center gap-3"
                  >
                    {loading ? (
